...
export default {
  ...
  methods: {
    ...
    async updateProfil() {
      ...
      await this.status(await JudgementAPI.mounted("PATCH", `users/${VueCookies.get('idUser')}`, body, "application/merge-patch+json", VueCookies.get('tokenUser')));
    }
  },
        ...
        <div class="row my-2">
          <div class="col-12">
            ...
            <input type="text" class="form-control" id="newName" v-model="newName"
                   :placeholder="user.username" minlength="3" required>
          </div>
        </div>
        <div class="row">
